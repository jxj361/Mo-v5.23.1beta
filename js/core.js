...
async function load() {
    // 自动迁移旧数据库（jxjDB → MoDB）
    const oldDBReq = indexedDB.open('jxjDB');
    oldDBReq.onsuccess = async function(e) {
      const oldDB = e.target.result;
      if (oldDB.objectStoreNames.contains('appData')) {
        const tx = oldDB.transaction('appData', 'readonly');
        const req = tx.objectStore('appData').get('appData');
        req.onsuccess = async function() {
          if (req.result) {
            const oldData = req.result.data;
            await saveToDB({ id: 'appData_core', data: oldData });
          }
        };
      }
    };
    ...
    // 迁移旧 localStorage 数据
    const oldTheme = localStorage.getItem('jxj_theme_backup');
    const oldChat = localStorage.getItem('jxj_chat_final_v9');
    if (oldTheme) { localStorage.setItem('Mo_theme_backup', oldTheme); }
    if (oldChat) { localStorage.setItem('Mo_chat_final_v9', oldChat); }
